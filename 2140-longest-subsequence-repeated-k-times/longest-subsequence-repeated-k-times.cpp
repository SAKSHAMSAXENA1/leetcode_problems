...
bool canGenerateSubsequences(string &ds,vector<bool> &canUse,
vector<int> &remCharFreqPerSubsequence, int &maxLen,string &res,string &s,int &k)
{
    if(ds.size()==maxLen)
    {
    ...
    for(int i=25;i>=0;i--)
    {
        ...
        if(canGenerateSubsequences(ds,canUse,remCharFreqPerSubsequence,maxLen,res,s,k))
        return true;
    ...
    string longestSubsequenceRepeatedK(string s, int k) {
        ...
        {
            string cur="";
            if(canGenerateSubsequences(cur,canUse,charFreqPerSubsq,subsequenceLen,res,s,k))
            break;
        }
